...
*************************************************/
Bzip_Compression::Bzip_Compression(length_type l) :
...
*************************************************/
void Bzip_Compression::write(const byte input[], length_type length)
   {
...
*************************************************/
void Bzip_Decompression::write(const byte input[], length_type length)
   {
   ...
   while(bz->stream.avail_in != 0)
      ...
      if(rc == BZ_STREAM_END)
         {
         length_type read_from_block = length - bz->stream.avail_in;
         start_msg();
