...
class MemoryRegion
   ...
   private:
      T* allocate(length_type n) const
         {
         return static_cast<T*>(alloc->allocate(sizeof(T)*n));
         }

      void deallocate(T* p, length_type n) const
         { alloc->deallocate(p, sizeof(T)*n); }

      mutable T* buf;
      mutable length_type used;
      mutable length_type allocated;
...
*************************************************/
template<typename T>
void MemoryRegion<T>::grow_to(length_type n) const
   {
