...
class Zlib_Alloc_Info
   {
   public:
      std::map<void*, length_type> current_allocs;
      Allocator* alloc;
...
void zlib_free(void* info_ptr, void* ptr)
   {
   Zlib_Alloc_Info* info = static_cast<Zlib_Alloc_Info*>(info_ptr);
   std::map<void*, length_type>::const_iterator i = info->current_allocs.find(ptr);
...
*************************************************/
Zlib_Compression::Zlib_Compression(length_type l) :
...
*************************************************/
void Zlib_Compression::write(const byte input[], length_type length)
   {
...
*************************************************/
void Zlib_Decompression::write(const byte input[], length_type length)
   {
   ...
   while(zlib->stream.avail_in != 0)
      ...
      if(rc == Z_STREAM_END)
         {
         length_type read_from_block = length - zlib->stream.avail_in;
         start_msg();
